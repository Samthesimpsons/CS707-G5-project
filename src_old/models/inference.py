...
class EpisodicInference:
    def __init__ (
            ...
            all_subs: bool # to extract subs for full episode (True) or event-specifi
    ):
    ...
    def build_prompt_text(self, qa_json: dict, episode_id: int) -> str:
        ...
        """
        prompt_parts = []
        instruction = "Select the correct multiple-choice answer option"
        if self.with_subs:
    ...
    ### BATCH JOB RUNNER
    ### ==========================================
    def generate_batch(self, max_new_tokens: int = 128) -> List[Dict]:
        if self.output_path is None:
            raise RuntimeError("Output Save Path for Results is Not Specified")
        now_str = datetime.now().strftime("%Y-%m-%d %H:%M")
        print(f"\n{'='*80}")
        print("Batch Job Inference: Initializing --- {now_str}")
        print(f"{'='*80}\n")
