...
#         if len(prompts) != len(video_lists):
#             raise ValueError("prompts and video_lists must have the same length")
    
#         messages_batch = []
#         for text_prompt, video_group in zip(prompts, video_lists):
#             # choose single video or list of frames
#             video_content = video_group[0] if len(video_group) == 1 else video_group
#             video_dict = {"type": "video", "video": video_content}
...
#     def run_inference_batch(self, text_prompts: list[str], video_paths: list[list], max_new_tokens):
#         # clips_data = [self.sample_clip_frames(vp) for vp in video_paths]
#         # clips_data = [self.sample_clip_frames(vp) for vp in video_paths]
#         # combined_clip = np.concatenate(clips_data, axis=0) if len(clips_data) > 1 else clips_data[0]
...
#         print("Begin Inference...")
#         ### Qwen-VL2.5 does not need to handle clips separately
#         messages_batch = self.generate_prompt_messages(
#             prompts=text_prompts,
#             video_lists=video_paths
#         )
...
#         # -----------------------------------
#         # Build batch: template per sample + collect vision inputs per sample
#         # -----------------------------------
#         texts = []
#         all_images = []
#         all_videos = []
...
#             # Each conv is a list[message dict]; create chat text for that one sample
#             t = self.processor.apply_chat_template(conv, tokenize=False, add_generation_prompt=True)
#             #print(t)
#             texts.append(t)
#             #print(conv)        
#             # Extract images/videos for that sample
...
#         inputs = self.processor(
#             text=texts,
#             #images=[],   # list[list[image_like]]
#             videos=all_videos,   # list[list[video_like]]
#             padding=True,
#             return_tensors="pt",
#         ).to(self.device)
...
### ======================= Qwen2.5-VL-7B-Instruct =======================
class QwenVL:
    def __init__ (self, model_path: str, config_path: str = "./config.toml"):
        self.model_path = model_path
...
class VideoLlava:
    ...
    def sample_and_collate(self, video_paths: list[str]) -> np.ndarray:
    ...
    def run_inference_batch(self, text_prompts: list, video_paths: list[list], max_new_tokens):
            #print(text_prompts)
            clips = []
            for v in video_paths:
                clip = self.sample_and_collate(v)
                #print(f'Clips shape: {clip.shape}')
                clips.append(clip)
            ...
            inputs = self.processor(
                ...
                #max_length=1024          # pick a cap that fits VRAM
            )
            inputs = inputs.to(self.device)
...
class VideoLLama3:
    ...
    def run_inference_batch(self, text_prompts: list, video_paths: list[list], max_new_tokens: int=128):
        text_prompt = text_prompts[0]
        #print(text_prompt, video_paths[0])
        ### Prepare for Inference
        messages = self.generate_prompt_message(text_prompt, video_paths[0])
        #print(messages)
...
class InternVL3:
    ...
    def run_inference_batch(self, text_prompts: list[str], video_paths: list[list], max_new_tokens:int =128):
        assert len(video_paths) == 1
        pixel_values, num_patches_list, video_prefix = InternVL3.process_video(video_paths[0][0], self.num_segments)
        generation_config = dict(max_new_tokens=max_new_tokens, do_sample=True)
        question = self.generate_prompt_message(text_prompts[0], video_prefix)
        #print(question)
        #print("Inference Start")
