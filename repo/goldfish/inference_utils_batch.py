...
def generate_batch(
    ...
    results_files = glob.glob(f'{output_path}/*.json')
    ...
    for ep_idx, qa_json in enumerate(tqdm(qa_pairs, desc="EpisodicInference Batch Run")):
        ...
        match = any(check_ep_id in f for f in results_files)
        #print(match)
        if match:
            ...
            if use_openai_embedding:
                embedding_path=f"new_workspace/open_ai_embedding/demo/season_1_{caption_type}/{video_name}.pkl"
            else:
                embedding_path=f"new_workspace/embedding/demo/season_1_{caption_type}/{video_name}.pkl"
