...
from goldfish.paths import DATA_DIR, LEGACY_WORKSPACE_DIR, WORKSPACE_DIR, ensure_dir
import pysrt
import chardet
...
from split_vtt_for_clips import split_vtt_for_clips
...
class GoldFish_LV:
    ...
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.workspace_dir = ensure_dir(WORKSPACE_DIR)
        self.legacy_workspace_dir = ensure_dir(LEGACY_WORKSPACE_DIR)
    ...
    def load_llama3_1_model(self):
        model_id = "/common/public/LLAMA3.1/Meta-Llama-3.1-8B-Instruct"
        bnb_config = BitsAndBytesConfig(
                    load_in_8bit=True,
                )
    ...
    def _youtube_download(self, url: str) -> str:
        try:
            ...
            tmp_dir = ensure_dir(self.legacy_workspace_dir / "tmp")
            output_path = tmp_dir / f"{video_id}.mp4"
            self.video_id=video_id
            video_stream.download(output_path=str(tmp_dir), filename=f"{video_id}.mp4")
    ...
    def get_subtitles(self, video_path) :
        ...
        audio_dir = ensure_dir(self.legacy_workspace_dir / "audio" / video_name)
        subtitle_dir = ensure_dir(self.legacy_workspace_dir / "subtitles" / video_name)
        # if the subtitles are already generated, return the path of the subtitles
        subtitle_path = subtitle_dir / f"{video_id}.vtt"
    ...
    def split_long_video_into_clips(self,video_path):
        ...
        tmp_save_path=ensure_dir(self.legacy_workspace_dir / "tmp" / "season_1" / self.video_name)
        print("tmp_save_path",tmp_save_path)

        if not any(tmp_save_path.iterdir()):
        ...
        videos_list = sorted(tmp_save_path.iterdir())
        return videos_list,tmp_save_path
        
    def long_inference_video(self, videos_list,tmp_save_path,subtitle_paths) -> Optional[str]:
        save_long_videos_path = ensure_dir(
            self.workspace_dir / "clips_summary" / "demo" / "season_1"
        )
        file_path = save_long_videos_path / f"{self.video_name}.json"
        ...
        if file_path.exists():
            ...
            for i, video in tqdm(enumerate(videos_list), desc="Inference video clips", total=len(videos_list)):
                clip_path = video if isinstance(video, Path) else Path(tmp_save_path) / video
                batch_video_paths.append(str(clip_path))
                ...
                previous_caption=""
                self.summary_instruction = f"The video is from season 1 {self.video_name} of famous TV show Friends.Please provide me with a detailed summary of the video content and try to be as descriptive as possible."
    ...
    def inference(self,video_path, use_subtitles=True, instruction="", number_of_neighbours=3):
        ...
        if video_duration > 180 :
            ...
            file_path=self.workspace_dir / "clips_summary" / "demo" / "season_1" / f"{video_name}.json"
            if not file_path.exists():
                print("Clips summary is not ready")
                videos_list,tmp_save_path=self.split_long_video_into_clips(video_path)
                subtitle_out_dir = ensure_dir(
                    self.legacy_workspace_dir / "subtitles" / "season_1" / self.video_name
                )
                subtitle_paths = split_vtt_for_clips(
                    full_vtt_path=str(DATA_DIR / "vtt_files" / "season_1" / f"{self.video_name}.vtt"),
                    out_dir=str(subtitle_out_dir),
                    clip_duration=80,
                    total_duration=video_duration,
                    num_clips=len(videos_list)
                )
                clips_summary = self.long_inference_video(videos_list,tmp_save_path,subtitle_paths)
            else: 
                print("External memory is ready")
            embedding_root = self.workspace_dir / (
                "open_ai_embedding" if self.args.use_openai_embedding else "embedding"
            )
            season_folder = ensure_dir(embedding_root / "demo" / "season_1")
            embedding_path=season_folder / f"{video_name}.pkl"
